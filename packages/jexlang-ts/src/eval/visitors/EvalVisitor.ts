...
export class EvalVisitor extends JexLangVisitor<MaybePromise<JexValue>> {
  ...
  private handlePromise<T>(
    ...
    handler: (resolved: T) => MaybePromise<T>
  ): MaybePromise<T> {
  ...
  private handlePromises<T, K extends T[]>(
    values: MaybePromise<T>[],
    handler: (...resolved: K) => MaybePromise<T>
  ): MaybePromise<T | T[]> {
  ...
  visitProgram = (
    ctx: JexLangParser.ProgramContext
    ...
    for (let i = 0; i < statements.length; i++) {
      if (hasPromise) {
        // Already in async mode, chain the next statement execution
        const currentIndex = i; // Capture the index
        result = (result as unknown as Promise<JexValue>).then(() => {
          return this.visit(statements[currentIndex]);
        });
      } else {
        // Execute the statement
        const statementResult = this.visit(statements[i]);

        if (statementResult instanceof Promise) {
          // This statement returned a Promise, switch to async mode
          hasPromise = true;
    ...
    if (hasPromise) {
      return (result as unknown as Promise<JexValue>)
        .then((resolved) => {
          cleanup();
  ...
  visitStatement = (
    ctx: JexLangParser.StatementContext
  ): MaybePromise<JexValue> => {
  ...
  visitVarDeclaration = (
    ctx: JexLangParser.VarDeclarationContext
  ): MaybePromise<JexValue> => {
    const varName = ctx.IDENTIFIER().getText();
    const varValue = ctx.singleExpression()
    ...
    return this.handlePromise(varValue, (resolvedValue) => {
      const globalScope = this.scope.resolveScopeByType("global");
      if (isGlobal && globalScope) {
        // if variable declared as global scope then declare in global scope
        globalScope.declareVariable(varName, resolvedValue, isConst);
      } else {
        // otherwise in current scope
  ...
  visitLiteralExpression = (
    ctx: JexLangParser.LiteralExpressionContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.literal());
  };

  visitBooleanLiteral = (
    ctx: JexLangParser.BooleanLiteralContext
  ): MaybePromise<JexValue> => {
    return ctx.getText().toLocaleLowerCase() === "true"; // Normalize boolean literals
  };

  visitNumberLiteral = (
    ctx: JexLangParser.NumberLiteralContext
  ): MaybePromise<JexValue> => {
    return toNumber(ctx.getText()); // all numbers are treated as floats
  };

  visitStringLiteral = (
    ctx: JexLangParser.StringLiteralContext
  ): MaybePromise<JexValue> => {
    return ctx.getText().slice(1, -1); // Remove quotes
  };

  visitNullLiteral = (
    ctx: JexLangParser.NullLiteralContext
  ): MaybePromise<JexValue> => {
    return null;
  };
  ...
  visitRepeatExpression = (
    ctx: JexLangParser.RepeatExpressionContext
  ): MaybePromise<JexValue> => {
  ...
  private handleNumericRepeat(
    count: number,
    block: JexLangParser.BlockContext
  ): MaybePromise<JexValue> {
    if (count < 0) {
      throw new JexLangRuntimeError(
        `Cannot repeat a block negative times: ${count}`
      );
    }
  ...
  private handleObjectRepeat(
    obj: Record<string, JexValue>,
    block: JexLangParser.BlockContext
  ): MaybePromise<JexValue> {
  ...
  visitEmptyStatement = (
    ctx: JexLangParser.EmptyStatementContext
  ): MaybePromise<JexValue> => {
    return null; // just return null for empty statements
  };

  visitExpressionStatement = (
    ctx: JexLangParser.ExpressionStatementContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.expressionSequence());
  };

  visitExpressionSequence = (
    ctx: JexLangParser.ExpressionSequenceContext
  ): MaybePromise<JexValue> => {
  ...
  visitArrayLiteralExpression = (
    ctx: JexLangParser.ArrayLiteralExpressionContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.arrayLiteral());
  };

  visitArrayLiteral = (
    ctx: JexLangParser.ArrayLiteralContext
  ): MaybePromise<JexValue> => {
  ...
  visitArrayElement = (
    ctx: JexLangParser.ArrayElementContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.singleExpression());
  };

  visitObjectLiteralExpression = (
    ctx: JexLangParser.ObjectLiteralExpressionContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.objectLiteral());
  };

  visitObjectLiteral = (
    ctx: JexLangParser.ObjectLiteralContext
  ): MaybePromise<JexValue> => {
  ...
  visitPropertyExpressionObjectProperty = (
    ctx: JexLangParser.PropertyExpressionObjectPropertyContext
  ): MaybePromise<JexValue> => {
  ...
  visitComputedPropertyExpressionObjectProperty = (
    ctx: JexLangParser.ComputedPropertyExpressionObjectPropertyContext
  ): MaybePromise<JexValue> => {
  ...
  visitShorthandPropertyExpressionObjectProperty = (
    ctx: JexLangParser.ShorthandPropertyExpressionObjectPropertyContext
  ): MaybePromise<JexValue> => {
  ...
  visitObjectPropertyName = (
    ctx: JexLangParser.ObjectPropertyNameContext
  ): MaybePromise<JexValue> => {
    if (ctx.STRING()) {
      return ctx.getText().slice(1, -1); // Remove quotes
    }
  ...
  visitIdentifierExpression = (
    ctx: JexLangParser.IdentifierExpressionContext
  ): MaybePromise<JexValue> => {
    const identifier = ctx.IDENTIFIER().getText();
  ...
  visitParenthesizedExpression = (
    ctx: JexLangParser.ParenthesizedExpressionContext
  ): MaybePromise<JexValue> => {
    return this.visit(ctx.expressionSequence());
  };

  visitAssignmentExpression = (
    ctx: JexLangParser.AssignmentExpressionContext
  ): MaybePromise<JexValue> => {
    const varName = ctx.IDENTIFIER().getText();
  ...
  visitBracketPropertyAssignment = (
    ctx: JexLangParser.BracketPropertyAssignmentContext
  ): MaybePromise<JexValue> => {
    ...
    return this.handlePromises(
      ...
      (object: Record<string, JexValue> | JexValue[], property, value) => {
        if (
          object != null &&
        ...
        throw new JexLangRuntimeError(
          `Cannot assign property on non-object or non-array value`
        );
      }
    );
  };

  visitDotPropertyAssignment = (
    ctx: JexLangParser.DotPropertyAssignmentContext
  ): MaybePromise<JexValue> => {
    const object = this.visit(ctx.singleExpression(0));
    const propertyName = this.visit(ctx.objectPropertyName());
    const value = this.visit(ctx.singleExpression(1));
  ...
  visitTernaryExpression = (
    ctx: JexLangParser.TernaryExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(
      ...
      (resolvedCondition) => {
        ...
        if (toBoolean(resolvedCondition)) {
          ...
          return this.handlePromise(
            this.visit(ctx.singleExpression(2)),
            (resolvedFalseBranch) => {
  ...
  visitShortTernaryExpression = (
    ctx: JexLangParser.ShortTernaryExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(
      this.visit(ctx.singleExpression(0)),
      (resolvedCondition) => {
  ...
  visitTransformExpression = (
    ctx: JexLangParser.TransformExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(this.visit(ctx.singleExpression()), (input) => {
      ...
      if (this.transformRegistry.has(transformName)) {
        return this.handlePromise(
          this.transformRegistry.transform(
            transformName,
            input,
      ...
      if (this.funcRegistry.has(transformName)) {
        return this.handlePromise(
          this.funcRegistry.call(transformName, [input], this.evaluatorContext),
          (output) => {
            return output;
          }
        );
  ...
  visitPowerExpression = (
    ctx: JexLangParser.PowerExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromises(
      [
        this.visit(ctx.singleExpression(0)),
  ...
  visitMultiplicativeExpression = (
    ctx: JexLangParser.MultiplicativeExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromises(
      [
        this.visit(ctx.singleExpression(0)),
      ...
      (left, right) => {
        ...
        throw new JexLangRuntimeError(
          `Unknown multiplicative operator ${ctx.getChild(1).getText()}`
        );
      }
    );
  };
  ...
  visitAdditiveExpression = (
    ctx: JexLangParser.AdditiveExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromises(
      [
        this.visit(ctx.singleExpression(0)),
      ...
      (left, right) => {
        ...
        throw new JexLangRuntimeError(
          `Unknown additive operator ${ctx.getChild(1).getText()}`
        );
      }
    );
  };
  ...
  visitRelationalExpression = (
    ctx: JexLangParser.RelationalExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromises(
      [
        this.visit(ctx.singleExpression(0)),
      ...
      (left, right) => {
        ...
        throw new JexLangRuntimeError(
          `Unknown relational operator ${ctx.getChild(1).getText()}`
        );
      }
    );
  };
  ...
  visitEqualityExpression = (
    ctx: JexLangParser.EqualityExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromises(
      [
        this.visit(ctx.singleExpression(0)),
      ...
      (left, right) => {
        ...
        throw new JexLangRuntimeError(
          `Unknown equality operator ${ctx.getChild(1).getText()}`
        );
      }
    );
  };
  ...
  visitLogicalAndExpression = (
    ctx: JexLangParser.LogicalAndExpressionContext
  ): MaybePromise<JexValue> => {
    if (ctx.AND()) {
    ...
    throw new JexLangRuntimeError(
      `Unknown logical operator ${ctx.getChild(1).getText()}`
    );
  };

  visitLogicalOrExpression = (
    ctx: JexLangParser.LogicalOrExpressionContext
  ): MaybePromise<JexValue> => {
    if (ctx.OR()) {
      return this.handlePromise(this.visit(ctx.singleExpression(0)), (left) => {
        const leftBool = toBoolean(left);
  ...
  visitUnaryExpression = (
    ctx: JexLangParser.UnaryExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(this.visit(ctx.singleExpression()), (value) => {
      ...
      throw new JexLangRuntimeError(
        `Unknown unary operator ${ctx.getChild(0).getText()}`
      );
    });
  };

  visitSquareRootExpression = (
    ctx: JexLangParser.SquareRootExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(this.visit(ctx.singleExpression()), (value) => {
  ...
  visitMemberDotExpression = (
    ctx: JexLangParser.MemberDotExpressionContext
  ): MaybePromise<JexValue> => {
  ...
  visitMemberIndexExpression = (
    ctx: JexLangParser.MemberIndexExpressionContext
  ): MaybePromise<JexValue> => {
    const obj = this.visit(ctx.singleExpression());
  ...
  visitFunctionCallExpression = (
    ctx: JexLangParser.FunctionCallExpressionContext
  ): MaybePromise<JexValue> => {
    ...
    const args: MaybePromise<JexValue[]> = ctx.arguments()
      ? (this.visit(ctx.arguments()) as MaybePromise<JexValue[]>)
      : [];
  ...
  visitArguments = (
    ctx: JexLangParser.ArgumentsContext
  ): MaybePromise<JexValue[]> => {
  ...
  visitArgument = (
    ctx: JexLangParser.ArgumentContext
  ): MaybePromise<JexValue> => {
  ...
  visitPrefixExpression = (
    ctx: JexLangParser.PrefixExpressionContext
  ): MaybePromise<JexValue> => {
    ...
    // Handle property access expressions (obj.prop or obj[prop])
      ...
      return this.handlePromises(
        [object, this.visit(propNameCtx)],
        (obj: Record<string, JexValue>, propName: JexValue) => {
          if (obj == null || typeof obj !== "object") {
            throw new JexLangRuntimeError(
              "Cannot use prefix operator on a non-object property"
            );
          }
          const currentValue = toNumber(obj[toString(propName)]);
          const newValue = ctx.INCREMENT()
            ? currentValue + 1
            : currentValue - 1;
      ...
      return this.handlePromises(
        [object, index],
        (obj: Record<string, JexValue>, idx: JexValue) => {
          if (obj == null || typeof obj !== "object") {
            throw new JexLangRuntimeError(
              "Cannot use prefix operator on a non-object index"
            );
          }
  ...
  visitPostfixExpression = (
    ctx: JexLangParser.PostfixExpressionContext
  ): MaybePromise<JexValue> => {
    const expr = ctx.singleExpression();
    ...
    // Handle property access expressions (obj.prop or obj[prop])
      ...
      return this.handlePromises(
        ...
        (obj: Record<string, JexValue>, propName: JexValue) => {
          if (obj == null || typeof obj !== "object") {
            throw new JexLangRuntimeError(
              "Cannot use postfix operator on a non-object property"
            );
          }
          const currentValue = toNumber(obj[toString(propName)]);
          const newValue = ctx.INCREMENT()
      ...
      return this.handlePromises(
        ...
        (obj: Record<string, JexValue>, idx: JexValue) => {
          if (obj == null || typeof obj !== "object") {
            throw new JexLangRuntimeError(
              "Cannot use postfix operator on a non-object index"
            );
          }
          if (Array.isArray(obj)) {
            const indexNumber = toNumber(idx);
  ...
  visitIfExpression = (
    ctx: JexLangParser.IfExpressionContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(
      ...
      (condition) => {
        ...
        if (ctx.elseIfStatement()) {
          // check for else if then execute else if
          return this.handlePromise(
  ...
  visitElseIfClause = (
    ctx: JexLangParser.ElseIfClauseContext
  ): MaybePromise<JexValue> => {
    return this.handlePromise(
      this.visit(ctx.expressionSequence()),
      (condition) => {
  ...
  visitElseClause = (
    ctx: JexLangParser.ElseClauseContext
  ): MaybePromise<JexValue> => {
